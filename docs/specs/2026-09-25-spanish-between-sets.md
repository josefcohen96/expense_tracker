...
- **Every card opens on the Hebrew alone** (added 2026-09-25): the Spanish is never on screen before the athlete has tried. Hint line `תגיד את זה בספרדית` (a new word: `מילה חדשה · נסה בספרדית, או הצג`); 🎤 button when speech recognition exists; button `הצג`.
- **Intro card, revealed:** the Spanish sentence large, `dir="ltr"`, target word emphasised, audio auto-plays; button `הבנתי` → records `{mode: "intro", grade: 2}`.
- **Recall card, revealed:** the Spanish sentence with the target emphasised, audio auto-plays, then four grade buttons in one row: `שוב · קשה · טוב · קל`. Tapping one records the review and shows the next card.
- **Said into the mic:** the answer is checked. Right (`Spanish.isCorrect`, Jaccard ≥ 0.6) → the Spanish is revealed with `✓ נכון` (`data-result="correct"`), the review is recorded as `טוב` at once, and the next card follows by itself after `ADVANCE.ms` (1.4 s) — no grade to pick. Wrong → the Spanish is revealed next to what was heard (`.sp-heard.is-wrong`, `data-result="wrong"`) and the card waits: `קשה` pre-selected on a recall, `הבנתי` on a new word.
...
- Recognition: `window.SpeechRecognition || window.webkitSpeechRecognition`, `lang = "es-ES"`, `interimResults = true` (iOS Safari hands over only interim results until it is stopped). The mic is a toggle: one tap listens ("⏹ סיימתי"), the next tap stops and grades what was heard; listening ends by itself after 8 s (`LISTEN.maxMs`) and, if the recogniser never reports its end, 1.5 s after being stopped (`LISTEN.settleMs`) whatever was heard is used — iOS Safari neither ends on silence nor reliably fires `onend`. Normalise both strings (lowercase, strip accents and punctuation), compare by token overlap: Jaccard ≥ 0.6 counts as right (recorded as `טוב`, next card by itself), lower reveals the answer and pre-selects `קשה`; the heard text is shown under the card in small type. Any error (no permission, no result) simply leaves the card on its prompt. Never send audio anywhere.
